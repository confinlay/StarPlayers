...
function initializePlayer() {
	renderSongList(); // Render song list immediately
...
function renderSongList() {
	...
	songs.forEach((song, index) => {
		...
		// Add loading state while assets load
		songElement.classList.add('loading');
		
		songElement.innerHTML = `
			<div class="song-info">
				<span class="song-title">${songTitles[song].title}</span>
				<span class="song-artist">${songTitles[song].artist}</span>
			</div>
			<i class="fas fa-star star-btn ${starPlayers.has(song) ? 'starred' : ''}"></i>
		`;

		const starBtn = songElement.querySelector('.star-btn');
		
		// Star button click handler with animation
		...
		if ((isShuffle && songs[index] === shuffledSongs[songIndex]) ||
			(starPlay && Array.from(starPlayers)[songIndex] === song) ||
			(!isShuffle && !starPlay && index === songIndex)) {
			songElement.classList.add('active');
		}

		// Click handler with feedback animation
		songElement.addEventListener('click', () => {
			songElement.classList.add('clicked');
			...
			setTimeout(() => {
				songElement.classList.remove('clicked');
			}, 200);
		});
...
function updateStarPlayersUI() {
	document.querySelectorAll('.song-item').forEach((item, index) => {
		...
		const isStarred = starPlayers.has(songs[index]);
		
		if (isStarred && !starBtn.classList.contains('starred')) {
			starBtn.classList.add('starred');
		} else if (!isStarred && starBtn.classList.contains('starred')) {
